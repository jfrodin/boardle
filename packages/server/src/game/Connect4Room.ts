...
import { getBestConnect4Move } from '../games/connect4/Connect4AI.js';
...
export class Connect4Room {
  readonly id: string;
  readonly mode: GameMode;
  readonly skill?: AiSkill;

  private state: Connect4GameState;
  ...
  constructor(mode: GameMode, skill?: AiSkill, aiDelayMs?: number) {
    ...
    this.state = createInitialConnect4State();
  }

  setOnEmpty(cb: () => void): void { this.onEmpty = cb; }
  ...
  handleMessage(ws: WebSocket, msg: ClientMessage): void {
    ...
    if (msg.type === 'CONNECT4_MOVE') {
      this.handleMove(player, msg.col);
  ...
  private handleMove(player: Player, col: number): void {
    ...
    this.clearTurnTimer();
    const move = { col };
    this.state = applyConnect4Move(this.state, move);

    if (this.state.status === 'FINISHED') {
      this.broadcast({ type: 'CONNECT4_GAME_OVER', state: this.state, lastMove: move });
      this.cleanupIfAi();
      return;
    }

    this.broadcast({ type: 'CONNECT4_STATE_UPDATE', state: this.state, lastMove: move });
  ...
  private triggerAiMove(): void {
    setTimeout(() => {
      ...
      const col = getBestConnect4Move(this.state, this.skill);
      const move = { col };
      this.state = applyConnect4Move(this.state, move);
  ...
  private startTurnTimer(): void {
    ...
    this.turnTimer = setTimeout(() => {
      ...
      this.state = { ...this.state, status: 'FINISHED', winner: timedOutSide === 'SOUTH' ? 'NORTH' : 'SOUTH' };
      this.broadcast({ type: 'CONNECT4_GAME_OVER', state: this.state });
    }, TURN_TIMEOUT_MS);
  }
  ...
  private cleanupIfAi(): void {
    if (this.mode === 'ai') { this.closed = true; this.onEmpty?.(); }
  }
  ...
  private handleLeave(ws: WebSocket): void {
    ...
    if (this.mode === 'online' && this.state.status === 'ACTIVE') {
      ...
      if (leavingPlayer) {
        ...
        this.broadcastExcept(ws, { type: 'CONNECT4_GAME_OVER', state: this.state });
      }
    }
    this.onEmpty?.();
    ws.close();
