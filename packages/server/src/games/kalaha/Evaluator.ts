import { type GameState, type PlayerSide } from '@boardly/shared';
import { SOUTH, NORTH, PITS_PER_SIDE } from '@boardly/shared';
...
export function evaluate(state: GameState): number {
  ...
  // Capture opportunities: bonus for each empty pit that has opponent stones opposite
  // (a stone landing there last would trigger a capture)
  let captureBonus = 0;
  for (let i = 0; i < PITS_PER_SIDE; i++) {
    const mirrorIndex = PITS_PER_SIDE - 1 - i;
    if (state.board.pits[SOUTH][i] === 0 && state.board.pits[NORTH][mirrorIndex] > 0) {
      captureBonus += 0.3;
    }
    if (state.board.pits[NORTH][i] === 0 && state.board.pits[SOUTH][mirrorIndex] > 0) {
      captureBonus -= 0.3;
    }
  }
