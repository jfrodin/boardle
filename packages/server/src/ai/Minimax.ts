import { type GameState, type Move } from '@kalaha/shared';
import { getLegalMoves, applyMove, SOUTH, NORTH, PITS_PER_SIDE } from '@kalaha/shared';
...
function orderMoves(state: GameState, moves: Move[]): Move[] {
  ...
  const scored = moves.map(move => {
    ...
    // Simplified: extra turn if stones == distToStore (ignoring wrap-around)
    const isExtraTurn = stones === distToStore;
    const mirrorPit = PITS_PER_SIDE - 1 - move.pitIndex;
    const oi = si === SOUTH ? NORTH : SOUTH;
    const isCapture =
      !isExtraTurn &&
      stones <= PITS_PER_SIDE + distToStore && // rough range check
      state.board.pits[si][move.pitIndex + stones - 1 < PITS_PER_SIDE ? move.pitIndex + stones - 1 : 0] === 0;
